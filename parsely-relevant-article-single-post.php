...
function my_plugin_enqueue_styles() {
    ...
    if (is_single()) {
        ...
        wp_enqueue_style('personalized-relevant-article-widget-style', plugins_url('/parsely-widget-style.css', __FILE__));
    }
}
...
if ($response === false) {
    ...
    $error = curl_error($ch);
    curl_close($ch);
    throw new Exception("cURL error: $error"); // Handle or log the error appropriately
} else {
    // Process successful response
    $parsedResponse = json_decode($response, true);
    if ($parsedResponse['success'] === false) {
        // Handle API-specific errors
        // echo "API error: " . $parsedResponse['message'];
    } else {
